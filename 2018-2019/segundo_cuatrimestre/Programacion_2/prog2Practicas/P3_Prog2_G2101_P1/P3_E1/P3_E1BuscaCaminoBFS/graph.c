...
Descripción: Es el graph.c de practicas anteriores
Autor: Junco de las Heras, Joaquin Murcia & Borja Velasco
Fecha: 25/03/19
Módulos propios que necesita: stack_fp.h y queue.h
...
Node *graph_findBreathSearch(Graph *g, int from_id, int to_id) {
    int origenId = find_node_index(g, from_id), destinoId = find_node_index(g, to_id), numNodos = graph_getNumberOfNodes(g);
    int i, *actual = NULL;
    Queue *q = NULL;
    Node *resultado = NULL;

    if (!g || origenId < 0 || origenId >= numNodos || destinoId < 0 || destinoId >= numNodos)return NULL;

    q = queue_ini(intDestroy, intCopy, intPrint);
    if (!q)return NULL;

    /*Resetea los antecesores*/
    for (i = 0; i < numNodos; i++) {
        node_setAntecesor(g->nodes[i], -1);
    }

    if (queue_insert(q, &origenId) == ERROR) {
        queue_destroy(q);
        return NULL;
    }

    node_setAntecesor(g->nodes[origenId], origenId);

    while (queue_isEmpty(q) == FALSE) {

        actual = queue_extract(q);

        for (i = 0; i < numNodos; i++) {

            if(!g->connections[*actual][i])continue;

            if (node_getAntecesor(g->nodes[i]) == -1) {
                node_setAntecesor(g->nodes[i], *actual);

                /*Si lo encuentra hace una copia del destino y la devuelve*/
                if (i == destinoId) {
                    resultado = node_copy(g->nodes[i]);
                    queue_destroy(q);
                    intDestroy(actual);
                    return resultado;
                }

                if (queue_insert(q, &i) == ERROR) {
                    queue_destroy(q);
                    intDestroy(actual);
                    return NULL;
                }
...
int graph_printCaminoBFS(FILE *f, Graph *g, int from_id, int to_id) {
    int origenId = find_node_index(g, from_id), destinoId = find_node_index(g, to_id), numNodos = graph_getNumberOfNodes(g);
    int *actual = NULL, previo, cont = 0;
    Node *bfsNode = NULL;
    Stack *s = NULL;

    if (!f || !g || origenId < 0 || origenId >= numNodos || destinoId < 0 || destinoId >= numNodos)return ERROR;

    /*Hace un BFS para quedarse con los antecesores y para acabar en caso de no conectividad*/
    bfsNode = graph_findBreathSearch(g, from_id, to_id);
    if (!bfsNode)return ERROR;
    node_destroy(bfsNode);

    /*Se crea una pila para revertir el orden de los caminos*/
    s = stack_ini(intDestroy, intCopy, intPrint);
    if (!s) {
        return ERROR;
    }

    previo = node_getAntecesor(g->nodes[destinoId]);

    if (stack_push(s, &destinoId) == ERROR) {
        stack_destroy(s);
        return ERROR;
    }

    while (previo != origenId) {
        if (stack_push(s, &previo) == ERROR) {
            stack_destroy(s);
            return ERROR;
        }

        previo = node_getAntecesor(g->nodes[previo]);
    ...
    while (stack_isEmpty(s) == FALSE) {
        actual = stack_pop(s);
        cont+=node_print(f, g->nodes[*actual]);
