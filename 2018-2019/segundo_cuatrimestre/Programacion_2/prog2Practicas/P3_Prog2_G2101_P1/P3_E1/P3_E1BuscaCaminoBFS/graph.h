...
/*DFS, devuelve NULL si no se ha podido encontrar con DFS el nodo con id: to_id y un puntero al nodo
 con id to_id si se ha podido encontrar con DFS*/
Node *graph_findDeepSearch (Graph *g, int from_id, int to_id);
/*Muestra un camino de nodos siempre y cuando haya conectividad entre ellos*/
void graph_printCaminoDFS (FILE *f, Graph *g, int from_id, int to_id);
/*DFS, devuelve NULL si no se ha podido encontrar con BFS el nodo con id: to_id y un puntero al nodo
 con id to_id si se ha podido encontrar con BFS*/
Node *graph_findBreathSearch (Graph *g, int from_id, int to_id);
/*Muestra el camino con menos aristas entre nodos siempre y cuando haya conectividad entre ellos.
