...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        # t[1] Un vector con 4 elementos, t[1][i] es True si se ha visitado la esquina self.corners[i], sino False.
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[nextx][nexty]:
                ...
                cost = 1
                # Si nextState es una esquina se marca como visitada (para ese estado de busqueda).
...
def cornersHeuristic(state, problem):
    ...
    # Si el estado es meta entonces el valor heuristico es 0.
    if problem.isGoalState(state):
        return 0

    manhattan_dist = util.manhattanDistance

    # Esquinas  no se han visitado todavia.
    corners_not_visited = []
    ...
    # Si solo falta 1 esquina entonces el valor heuristico es la distancia Manhattan del estado a la esquina.
    if n == 1:
        return manhattan_dist(state[0], corners_not_visited[0])

    # Si falta mas de una esquina por descubrir, el valor heuristico, que se guarda en min_distance
    # es el minimo valor entre todos los caminos manhattan posibles.
    # Por ejemplo, si hay 4 esquinas sin visitar, un camino manhattan posible es ir de
    # la posicion actual (del estado) a la primera esquina, de la primera esquina a la segunda etc
    # usando como distancia la manhattan porque sabemos que es <= que la real, y como tomamos el minimo
    # nos aseguramos de que es admisible y consistente.

    # El camino es min(state[0] -> corner[0] -> corner[1], state[0] -> corner[1] -> corner[0]).
    if n == 2:
            ...
            for j in range(n):
                ...
                for k in range(n):
                    ...
                    min_distance = min(min_distance,
                                       manhattan_dist(state[0], corners_not_visited[i]) +
                                       manhattan_dist(corners_not_visited[i], corners_not_visited[j]) +
                                       manhattan_dist(corners_not_visited[j], corners_not_visited[k]))
                ...
                for k in range(n):
                    ...
                    for z in range(n):
                        ...
                        min_distance = min(min_distance,
                                           ...
                                           manhattan_dist(corners_not_visited[i], corners_not_visited[j]) +
                                           manhattan_dist(corners_not_visited[j], corners_not_visited[k]) +
                                           manhattan_dist(corners_not_visited[k], corners_not_visited[z]))
