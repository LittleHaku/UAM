...
#ficheros salida
#datos
fDATnr=cache_Directa_normal_read.dat
fDATnw=cache_Directa_normal_write.dat
fDATtr=cache_Directa_trasp_read.dat
fDATtw=cache_Directa_trasp_write.dat

#gráficas
fPNGnr=cache_Directa_lectura_normal.png
fPNGnw=cache_Directa_escritura_normal.png
fPNGtr=cache_Directa_lectura_trasp.png
fPNGtw=cache_Directa_escritura_trasp.png
...
for ((TamCache = TamCacheIni; TamCache <= TamCacheFin; TamCache += TamCache)); do
	...
	for ((N = Ninicio; N <= Nfinal; N += Npaso)); do
		...
		nr+=($(cat "cache_Directa_$TamCache.dat" | head -n $iteracion | tail -n 1 | awk '{print $2}')) #se obtiene D1r del programa normal
		nw+=($(cat "cache_Directa_$TamCache.dat" | head -n $iteracion | tail -n 1 | awk '{print $3}')) #se obtiene D1w del programa normal
		trr+=($(cat "cache_Directa_$TamCache.dat" | head -n $iteracion | tail -n 1 | awk '{print $4}')) #se obtiene D1r del programa trasp
		tw+=($(cat "cache_Directa_$TamCache.dat" | head -n $iteracion | tail -n 1 | awk '{print $5}')) #se obtiene D1w del programa trasp
		tamMatriz+=($N)
