/*********************************************************/
/* Calculo PI: Metodo de integracion - Version OpenMP	 */
/* false sharing 					                     */
/*********************************************************/
// Junco de las Heras Valenzuela junco.heras@estudiante.uam.es
// Marta Vaquerizo Nuñez marta.vaquerizo@estudiante.uam.es
#include <sys/time.h>
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
...
int main(int argc, char *argv[])
{
    ...
    f = popen
        ("getconf -a | grep CACHE | grep \"3_CACHE_LINESIZE\" | awk '{print $2}'",
         "r");
    fgets(buf, sizeof(buf), f);
    pclose(f);
    linesz = atoi(buf);
    padsz = linesz / datasz; // Linea 32 (La que se modifica)
