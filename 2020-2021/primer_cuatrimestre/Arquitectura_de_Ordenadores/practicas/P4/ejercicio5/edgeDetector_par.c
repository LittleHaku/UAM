...
float *gaussian_kernel(int ksize, double sigma)
{
    ...
    for (j = 0; j < ksize; j++) {
        for (i = 0; i < ksize; i++) {
            ...
            gauss[i + ksize * j] =
                (GAUSSIAN_K *
                 exp(((pow(x, 2) + pow(y, 2)) / ((2 * pow(sigma, 2)))) * (-1)));
            sum += gauss[i + ksize * j];
        }
    }
...
static inline void
getRGB(uint8_t * im, int width, int height, int nchannels, int x, int y,
       int *r, int *g, int *b)
{
...
{
    int width, height, nchannels, nprocs;
    ...
    for (int file_i = 1; file_i < nargs; file_i++) {
        ...
        asprintf(&denoised_image_filename, "%s_grad_denoised.jpg",
                 argv[file_i]);
        ...
        if (nchannels != 3 && nchannels != 4) {
            printf
                ("[error] Num of channels=%d not supported. Only three (RGB), four (RGBA) are supported.\n",
                 nchannels);
            continue;
        }
        ...
        for (int j = 0; j < height; j++) {
            #pragma omp parallel for firstprivate(width, r, g, b, grey_image)
...
#define PIXEL_GREY(x, y) (grey_image[(x) + (y)*width])
        ...
        for (int i = 1; i < width - 1; i++) {
            #pragma omp parallel for firstprivate(height, width_edges)
            for (int j = 1; j < height - 1; j++) {
                int x = i - 1;
                int y = j - 1;
                float a =
                    (PIXEL_GREY(i - 1, j - 1) +
                     PIXEL_GREY(i - 1, j) * 2 + PIXEL_GREY(i - 1,
                                                           j + 1) -
                     (PIXEL_GREY(i + 1, j - 1) +
                      PIXEL_GREY(i + 1, j) * 2 + PIXEL_GREY(i + 1,
                                                            j + 1)));
                float b =
                    (PIXEL_GREY(i - 1, j - 1) +
                     PIXEL_GREY(i, j - 1) * 2 + PIXEL_GREY(i + 1,
                                                           j - 1) -
                     (PIXEL_GREY(i - 1, j + 1) +
                      PIXEL_GREY(i - 1, j + 1) * 2 + PIXEL_GREY(i - 1,
                                                                j + 1)));
...
#ifdef WRITEONTHEGO
        stbi_write_jpg(grad_image_filename, width - 2, height - 2, 1,
                       edges, 10);
        free(grey_image);
#endif
...
#define PIXEL_EDGES(x, y) (edges[(x) + (y)*width_edges]);
        ...
        if (TYPE_FILTER == MEDIAN) {
            ...
            for (int i = radius; i < width_edges - radius; i++) {
                #pragma omp parallel for firstprivate(height_edges, x, y, array, edges_denoised)
                for (int j = radius; j < height_edges - radius; j++) {
                    y = j - radius;
                    x = i - radius;
                    k = 0;
                    //#pragma omp parallel for firstprivate(array, k)
                    for (int p1 = i - radius; p1 <= i + radius; p1++) {
                        for (int p2 = j - radius; p2 <= j + radius; p2++) {
                            array[k++] = PIXEL_EDGES(p1, p2);
                        }
                    }
                    qsort(array, (2 * radius + 1) * (2 * radius + 1),
                          1, comparate);

                    edges_denoised[x + y * width_denoised] =
                        array[(2 * radius + 1) * (2 * radius +
                                                  1) / 2] > 50 ? 255 : 0;
                }
            }
            ...
            //Bloque 4
            for (int i = radius; i < width_edges - radius; i++) {
                #pragma omp parallel for firstprivate(height_edges, x, y, array, edges_denoised, sum, width_denoised, kernel)
                for (int j = radius; j < height_edges - radius; j++) {
                    x = i - radius;
                    y = j - radius;
                    sum = 0;
                      ...
                      for (int p1 = 0; p1 <= 2 * radius; p1++) {
                            ...
                            sum +=
                                kernel[p1 +
                                       p2 * (2 * radius +
                                             1)] *
                                PIXEL_EDGES(i - radius + p1, j - radius + p2);
        ...
        stbi_write_jpg(denoised_image_filename, width_denoised,
                       height_denoised, 1, edges_denoised, 10);
        free(edges_denoised);
        free(edges);
#endif
...
#ifndef WRITEONTHEGO
        stbi_write_jpg(grey_image_filename, width, height, 1, grey_image, 10);
        stbi_write_jpg(grad_image_filename, width_edges, height_edges, 1,
                       edges, 10);
        stbi_write_jpg(denoised_image_filename, width_denoised,
                       height_denoised, 1, edges_denoised, 10);
        free(rgb_image);
