...
NO_PROMISC = 0
TO_MS = 10
#Tamaño de una cabecera Ethernet
TAM_CABECERA = 14
...
# Variables globales para startEthernetLevel.
...
def process_Ethernet_frame(us:ctypes.c_void_p,header:pcap_pkthdr,data:bytes) -> None:
    ...
    #se extraen los campos de ethernet
    destino   = data[0:6]
    origen    = data[6:12]
    ...
    #se comprueba si existe una función de callback de nivel superior asociada al Ethertype de la trama usando el diccionario upperProtos
    if upperProtos != None and upperProtos.get(int(ethertype.hex(), 16)) != None:
        upperProtos.get(int(ethertype.hex(), 16))(us,header, data[14:],origen)  
...
def registerCallback(callback_func: Callable[[ctypes.c_void_p,pcap_pkthdr,bytes],None], ethertype:int) -> None:
    ...
    #se añade al diccionario la asociacion de la funcion callback al ethertype
    if upperProtos is not None:
...
def startEthernetLevel(interface:str) -> int:
    ...
    # Se comprueba si ya está inicializado el nivel Ethernet.
    if levelInitialized is not False:
        return -1
    ...
    if handle is None:
        print("Error pcap_open_live" + str(errbuf))
        return -1
    ...
    # Se actualiza el estado de la funcion.
...
def stopEthernetLevel()->int:
    ...
    # Se actualiza el estado del nivel.
    levelInitialized = False
...
def sendEthernetFrame(data:bytes,leng:int,etherType:int,dstMac:bytes) -> int:
    '''
        ...
        Argumentos:
            ...
            -leng: longitud de los datos útiles expresada en bytes
    ...
    # Se contruye la trama Ethernet.
    trama = dstMac + macAddress + etherType.to_bytes(2, 'big') + data

    tam = len(trama)
    if tam > ETH_FRAME_MAX:
        return -1
    
    # Si la trama es muy pequeña, se rellena con 0's.
    if tam < ETH_FRAME_MIN:
        trama += bytearray(ETH_FRAME_MIN - tam) 
        tam = ETH_FRAME_MIN

    

    # Se envia la trama.
    ret = pcap_inject(handle, trama, tam)
    if ret != tam:
        print("Error en pcap_inject, se inyectan " + str(ret) + " de " + str(tam))
        return -1
