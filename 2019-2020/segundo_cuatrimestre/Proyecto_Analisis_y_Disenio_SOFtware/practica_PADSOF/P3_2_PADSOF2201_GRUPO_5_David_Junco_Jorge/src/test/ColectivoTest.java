...
import modelo.colectivoCiudadano.*;
...
public class ColectivoTest {
	...
	if (h.contains(c2) == false) fail("Fallo en Add ciudadano");
	if (h2.contains(co2) == false) fail("Fallo en Add colectivo");

	Ciudadano c3 = new Ciudadano("ciu3", "ciu3", "12343");
	co2.add(c3);
	co.add(c3);
	h = co.getDirectosCiudadanos();
	if (h.contains(c3)) fail("Fallo en Add mas de los que debe, col hijo");

	Ciudadano c4 = new Ciudadano("ciu4", "ciu4", "12344");
	co.add(c4);
	co2.add(c4);
	h = co2.getDirectosCiudadanos();
	if (h.contains(c4)) fail("Fallo en Add mas de los que debe, col padre");
    }
	...
	try {
	    co.remove(c);
	    fail("No se ha recibido una excepcion");
	} catch (Exception ex) {

	}
	Set<Ciudadano> h = co.getDirectosCiudadanos();
	if (h.contains(c) == false) fail("Fallo en remove, no se puede eliminar al representante");
	Ciudadano c2 = new Ciudadano("ciu2", "ciu2", "12342");
	co.add(c2);
	...
	if (h2.contains(c2) == true) fail("Fallo en remove, no se borra bien");
    }
	...
	if (co.getRepresentante().equals(c) == false) fail("Fallo en representante");
    }
