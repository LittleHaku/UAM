...
public abstract class Proyecto implements Comparable<Proyecto>, Serializable {
    ...
    public static final int MAX_DESCR_LEN = 500;
	...
	/**
	 * El proyecto reune los apoyos suficientes para ser enviado
	 */
	ENVIABLE("Enviable"),
	/**
     ...
     * 
     * @throws ValueTooLongException Si la longitud del titulo o la descripción es
     *                               demasiado larga
     * 
     */
	...
	LocalDate date = FechaSimulada.getHoy();

	if (titulo.length() > MAX_TITLE_LEN) {
	    throw new ValueTooLongException("Titulo", Proyecto.MAX_TITLE_LEN);
	}
	if (descripcion.length() > MAX_DESCR_LEN) {
	    throw new ValueTooLongException("Descripción", Proyecto.MAX_DESCR_LEN);
	}
	...
	id_siguiente++;
	apoyos.add(creador);
	subscritos.add(creador.getRepresentante());
	...
	if (this.getEstado() != EstadoProyecto.ENVIABLE) return false;
	...
	if (estado == EstadoProyecto.ENVIADO) {
		...
		if (importeAdjudicado > 0) this.setEstado(EstadoProyecto.ACEPTADO);
		else this.setEstado(EstadoProyecto.RECHAZADO);
	    }
	...
	if (!isApoyable()) return false;
	this.ultimoApoyo = FechaSimulada.getHoy();

	this.apoyos.add(e);
	e.apoyaProyecto(this);

	this.actualizarEnviable();
    ...
    /**
     * Comprueba si el proyecto se puede enviar, si se puede, cambia su estado a
     * ENVIABLE, o viceversa
     * <p>
     * Un proyecto se puede enviar si tiene el minimo numero de apoyos
     * 
     */
    public void actualizarEnviable() {
	if (this.estado == EstadoProyecto.EN_VOTACION) {
	    if (this.getNumApoyos() >= Aplicacion.getApoyosMinimos()) {
		this.setEstado(EstadoProyecto.ENVIABLE);
	    }
	}
	if (this.estado.equals(EstadoProyecto.ENVIABLE)) {
	    if (this.getNumApoyos() < Aplicacion.getApoyosMinimos()) {
		this.setEstado(EstadoProyecto.EN_VOTACION);
	    }
	}
	...
	boolean cad_old = caducado;
	caducado = FechaSimulada.getHoy().isAfter(ultimoApoyo.plusDays(DIAS_PARA_CADUCAR - 1));
	if (cad_old != caducado) {
	    this.subscritos.forEach(c -> c.addNotificacion(new NotificacionCambioEstado(this, "Caducado")));
	}
    }
     ...
     * 
     * @return <code> true </code> si son iguales (mismo id) <br>
...
class SolicitudProyecto implements GrantRequest {
	...
	if (p instanceof ProySocial) return GrantRequest.ProjectKind.Social;
