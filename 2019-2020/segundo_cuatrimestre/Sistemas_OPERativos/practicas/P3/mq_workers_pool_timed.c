 ...
 * Grupo: 2201
 * Fecha: 04-04-2020
 * Descripcion: Código de contar caracteres, cola no bloqueante con una alarma de tiempo
 * */
...
/*manejador de las señales SIGUSR2, SIGTERM y SIGALRM*/
...
int f_hijo(pid_t ppid, char* name, char* c) {
    ...
    while (got_term == 0) {
        if (got_alrm == 1) {
            got_alrm = 0;
            printf("El hijo con pid %d no es necesario\n", getpid());
            mq_close(queue);
            return EXIT_SUCCESS;
        }
